    ...
    return(
        <div className={`navbar h-100 p-0 m-0 d-flex flex-column position-relative ${isMinimize ? 'minimize-bar' : 'navbar'}`}>
            ...
            <div className="log-out w-100 p-3 m-0 "style={{height: '5rem', borderTop: '2px solid #E9F1FA'}}>
                {isMinimize ? <i class="bi-logout bi-door-open-fill fs-3"></i> : <button onClick={handleLogout} type="button" className="btn-logout w-100 h-100"><i className="bi-full-logout bi-door-open-fill fs-5"></i>LOG OUT</button>} 
            </div>
        </div>
    )
}
